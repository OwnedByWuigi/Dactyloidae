                                      ...
                                      char const *const ** aCodecList)
{
  ...
  static char const *const h264VideoCodecs[] = {
    "avc1",
    "h264",
    nullptr
  };
  if ((aType.EqualsASCII("video/h264") ||
       aType.EqualsASCII("video/avc")) &&
      CanDecodeH264UsingDirectShow()) {
    if (aCodecList) {
      *aCodecList = h264VideoCodecs;
    }
    return true;
  }
...
DirectShowDecoder::IsEnabled()
{
  return Preferences::GetBool("media.directshow.enabled") &&
         (CanDecodeMP3UsingDirectShow() || CanDecodeH264UsingDirectShow());
}
