...
AudioSinkInputPin::GetMediaType(int aPosition, MediaType* aOutMediaType)
{
  ...
  aOutMediaType->SetFormatType(&FORMAT_WaveFormatEx);
  ...
  if (aPosition == 1) {
    aOutMediaType->SetType(&MEDIATYPE_Video);
    aOutMediaType->SetSubtype(&MEDIASUBTYPE_YUY2);
    aOutMediaType->SetFormatType(&FORMAT_VideoInfo);
    aOutMediaType->SetTemporalCompression(FALSE);
    return S_OK;
  }
  return S_FALSE;
