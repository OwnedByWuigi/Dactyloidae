  ...
  while (filters->Next(1, getter_AddRefs(moniker), &fetched) == S_OK) {
    RefPtr<IPropertyBag> bag;
      ...
      if (SUCCEEDED(bag->Read(L"FriendlyName", &value, nullptr)) &&
          ...
          (wcsstr(value.bstrVal, L"H264") ||
           wcsstr(value.bstrVal, L"H.264") ||
           wcsstr(value.bstrVal, L"AVC"))) {
        VariantClear(&value);
        return true;
      }
      VariantClear(&value);
...
HRESULT
AddH264DecoderFilter(IGraphBuilder* aGraph, IBaseFilter** aOutFilter)
{
  NS_ENSURE_TRUE(aGraph && aOutFilter, E_POINTER);
  *aOutFilter = nullptr;
  RefPtr<ICreateDevEnum> devEnum;
  HRESULT hr = CoCreateInstance(CLSID_SystemDeviceEnum, nullptr,
  ...
  hr = devEnum->CreateClassEnumerator(CLSID_LegacyAmFilterCategory,
                                      getter_AddRefs(filters), 0);
  NS_ENSURE_TRUE(hr == S_OK, VFW_E_NOT_FOUND);
  RefPtr<IMoniker> moniker;
  ULONG fetched = 0;
  while (filters->Next(1, getter_AddRefs(moniker), &fetched) == S_OK) {
    RefPtr<IPropertyBag> bag;
    ...
    VariantClear(&value);
    if (h264 && SUCCEEDED(moniker->BindToObject(nullptr, nullptr, IID_IBaseFilter,
                                                reinterpret_cast<void**>(aOutFilter)))) {
      hr = aGraph->AddFilter(*aOutFilter, L"DirectShow H.264 decoder");
      if (SUCCEEDED(hr)) {
        return hr;
      }
      (*aOutFilter)->Release();
